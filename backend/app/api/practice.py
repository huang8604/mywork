...
from app.services.system_settings import audio_runtime_settings, resolve_audio_provider
...
def list_sessions(
    ...
    if status is None:
        filters.append(PracticeSession.status != "archived")
    for column, value in (
...
def get_session_item_audio(
    ...
    if language == "zh":
        audio = generate_chinese_audio(
            item.snapshot_cn_meaning,
...
def get_number_audio(
    ...
    """Serve a number-announcement clip for online dictation.

    English clips are pre-generated by ``POST /words/audio/generate-numbers``.
    Chinese clips are generated and cached on first authenticated playback.
    """
    settings = audio_runtime_settings(db)
    audio = number_audio_file(n, settings=settings, language=language)
...
def create_round(
    ...
    db.add(round_)
    db.flush()
    # Starting any review round moves the worksheet into the explicit in-progress state.
