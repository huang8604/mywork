...
async function installApi(page: Page) {
  ...
  await page.route('**/api/v1/**', async (route: Route) => {
    ...
    if (path === '/api/v1/reviews/today' && method === 'GET') return route.fulfill({ json: envelope({ date: '2026-07-20', timezone: 'Asia/Shanghai', counts: { known: 1, unknown: 0, skipped: 0, total: 1 }, items: [{ review_id: 9, round_id: 3, session_id: 2, session_title: '期末冲刺', word_id: 1, en_word: word.en_word, phonetic: word.phonetic, cn_meaning: word.cn_meaning, status: 'known', reviewed_at: '2026-07-20T02:00:00Z' }] }) })
...
test('word CRUD autofill and English-only import remain usable', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '导入 / 导出' }).click(); await expect(page.getByRole('radio', { name: '更新重复' })).toBeChecked(); await expect(page.getByRole('radio', { name: 'AI 补充' })).toBeChecked(); await page.getByRole('button', { name: '查看 JSON 字段模板' }).click(); await expect(page.getByRole('heading', { name: 'JSON 导入字段模板' })).toBeVisible(); await expect(page.locator('.json-template')).toContainText('example_sentence'); await page.keyboard.press('Escape'); await page.getByLabel('英文单词列表').fill('focus\ncamera'); await page.getByRole('button', { name: '开始导入' }).click(); await expect(page.getByText('词典命中')).toBeVisible()
})

test('online review selects a recent named sheet, shows today results, and permits correction', async ({ page }) => {
  await page.goto('/review'); await expect(page.locator('.session-choice')).toHaveCount(3); await expect(page.getByRole('heading', { name: '今日已完成' })).toBeVisible(); await expect(page.getByText(/期末冲刺.*\d{2}:\d{2}/)).toBeVisible(); await page.getByRole('button', { name: /期末冲刺/ }).click(); await page.getByRole('button', { name: '开始在线复习' }).click(); await page.getByRole('button', { name: /^认识/ }).click(); await expect(page.getByText('已记录：认识')).toBeVisible()
