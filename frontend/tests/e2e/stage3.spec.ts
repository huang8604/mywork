...
async function installApi(page: Page) {
  ...
  await page.route('**/api/v1/**', async (route: Route) => {
    ...
    if (path === '/api/v1/words/import/progress' && method === 'GET') return route.fulfill({ json: envelope({ state: 'idle', total: 1, processed: 1, created: 1, updated: 0, skipped: 0, failed: 0, unresolved: 0, unresolved_words: [], resolved: [], dictionary_matches: 1, audio_generation: { queued: 0 }, finished: true }) })
    if (path === '/api/v1/words/import') return route.fulfill({ json: envelope({ state: 'running', total: 1, processed: 0, created: 0, updated: 0, skipped: 0, failed: 0, unresolved: 0, unresolved_words: [], resolved: [], dictionary_matches: 0, audio_generation: { queued: 0 }, finished: false }) })
...
async function expectTouchTargets(page: Page, selector: string, minHeight = 48, minWidth = 44) {
  ...
  expect(sizes.every(size => size.height >= minHeight && size.width >= minWidth)).toBe(true)
}
...
test('activity heatmap and dictation defaults expose the new interaction model', async ({ page }) => {
  ...
  await expectTouchTargets(page, '.mode-toggle .mode-option', 52)
  await page.locator('.mode-option').filter({ hasText: '在线默写' }).click()
  ...
  await expectTouchTargets(page, '.head-right .el-select__wrapper, .head-right .el-button', 48)
  await page.locator('.head-right .el-select__wrapper').click()
  ...
  await expectNoHorizontalOverflow(page)
  await expectTouchTargets(page, '.runner-controls .el-button', 52)
...
test('online review hides results while answering and shows a separate result page when complete', async ({ page }) => {
  await page.goto('/review'); await expect(page.locator('.session-choice')).toHaveCount(2); await expect(page.getByText('周末巩固')).toHaveCount(0); await expect(page.getByRole('heading', { name: '今日已完成' })).toBeVisible(); await expect(page.getByText(/期末冲刺.*\d{2}:\d{2}/)).toBeVisible(); await page.getByRole('button', { name: /期末冲刺/ }).click(); await page.getByRole('button', { name: '开始在线复习' }).click(); await expect(page.getByRole('heading', { name: '今日已完成' })).toHaveCount(0); await expect(page.getByRole('button', { name: '退出复习' })).toBeVisible(); await page.getByRole('button', { name: '退出复习' }).click(); await expect(page.getByRole('heading', { name: '选择复习表' })).toBeVisible(); await expect(page.locator('.flash-card')).toHaveCount(0); await page.getByRole('button', { name: '开始在线复习' }).click(); await expectTouchTargets(page, '.result-selector .result-button', 56); await page.getByRole('button', { name: /^认识/ }).click(); await expect(page.locator('.finish-summary')).toContainText('本轮已完成'); await expect(page.locator('.flash-card')).toHaveCount(0); await expect(page.getByRole('heading', { name: '今日已完成' })).toBeVisible(); await expect(page.getByText('学员：student')).toBeVisible()
