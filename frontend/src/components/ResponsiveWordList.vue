...
defineEmits<{ edit: [word: Word]; remove: [word: Word]; reset: [word: Word]; review: [word: Word]; playAudio: [word: Word]; generateAudio: [word: Word] }>()
...
<template>
  <div class="desktop-word-table local-scroll">
    <el-table :data="words" row-key="id" style="min-width:1120px">
      ...
      <el-table-column label="音频" width="95"><template #default="{row}"><el-tag v-if="row.audio_path" size="small" type="success" effect="plain">已生成</el-tag><el-tag v-else size="small" type="info" effect="plain">未生成</el-tag></template></el-table-column>
  ...
  <div class="mobile-word-cards"><WordCard v-for="word in words" :key="word.id" :word="word" @edit="$emit('edit',word)" @remove="$emit('remove',word)" @reset="$emit('reset',word)" @review="$emit('review',word)" @play-audio="$emit('playAudio',word)" @generate-audio="$emit('generateAudio',word)" /></div>
</template>
