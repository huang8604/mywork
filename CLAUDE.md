...
- **volc** (豆包 `doubao-seed-tts-2.0`): the **agent-plan** endpoint `POST {VOLC_TTS_BASE_URL}/api/v3/plan/tts/unidirectional?api_key=<key>` (the api key MUST go in the query string) with header `X-Api-Resource-Id: seed-tts-2.0` and body `{"req_params":{text, speaker (string), audio_params:{format, speed_ratio}}}`. The `speaker` id (`VOLC_TTS_VOICE`) must be one bound to the seed-tts-2.0 resource on the account — the built-in `BVxxx` voices return `55000000 resource ID is mismatched with speaker related resource`; copy a valid speaker id from the 火山方舟 console. Requires the model be activated in the Ark console (配置模型 + 开启超额后付费). The success audio field is parsed defensively (`_decode_audio`).
