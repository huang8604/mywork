...
- **默认交付动作**：用户要求实现或修复且未明确要求“只检查/不发布”时，完成改动和本地验证后，默认将本次范围内的修改提交并使用上述脚本推送，继续等待并核验对应的 GitHub Actions；不再额外等待用户提示。若本地检查因代码问题失败则先修复，若仅因环境缺少依赖等外部条件受阻，可在明确记录阻塞项后提交推送并以 CI 结果为准。
...
Words carry server-generated MP3 pronunciation metadata (`audio_path`, `audio_format`, `audio_provider`, `audio_model`, `audio_voice`, `audio_generated_at`, `audio_bytes`). The System page exposes two independently saved connections — 豆包 (`volc`) and Mimo — with URL + write-only API key only; model and voice remain server-side defaults and are shown with generated results. 豆包 is the default, and a provider error automatically retries with Mimo for that request without changing the next run's default. The API key is masked in responses and never sent to Vue.

- **豆包**: `POST https://openspeech.bytedance.com/api/v3/plan/tts/unidirectional?api_key=...` with `X-Api-Resource-Id: seed-tts-2.0`; the JSON body is the agent-plan `req_params` contract and the response is reassembled from NDJSON base64 chunks.
- **Mimo**: the saved `/v1` root is extended to `/chat/completions`; the OpenAI-compatible response is read from `choices[0].message.audio.data`.

Admin audio routes (all `words:*`, never exposed to students — students lack `words:read`): word generation, batch generation, number announcements, and progress remain background operations. They resolve the selected provider, record the actual provider/model/voice (including fallback), and expose a test-speech action plus a manual dictionary “regenerate with current service” action. The API keys are never returned to Vue.
...
**Runtime TTS settings (0011):** the System page persists separate `mimo_*` and `volc_*` URL/key overrides in the `SystemAudioSetting` singleton, plus the default-provider and import auto-generate toggle (`auto_generate_on_import`, overriding `TTS_AUTO_GENERATE_ON_IMPORT`). Empty keys preserve the existing secret. The former `custom_*` columns are retained only for one-release migration compatibility and are not exposed as account settings.
...
The `/system` route (`meta.roles: ['admin']`) is the admin console: the persistent issue/requirement notebook, separate 豆包/Mimo TTS URL + masked/write-only API keys with test and import auto-generation controls, local-dictionary shared audio generation (progress, pause/resume, retry/quota wait, and manual regeneration), API-client/token management (create, rotate, revoke, adjust scopes — the plaintext token is shown only on create/rotate), and one-click SQLite full-backup download (`GET /api/v1/system/backup`). It replaces the CLI lifecycle scripts for day-to-day admin.
...
`Settings.from_env()` (`core/config.py`, `lru_cache`d) is the single source for config. Required for startup: `API_TOKEN_PEPPER` or `API_TOKEN_PEPPER_FILE` (≥32 bytes). Production also needs `DATABASE_URL`, `PUBLIC_BASE_URL`, `TRUSTED_HOSTS`, `TRUSTED_PROXY_CIDRS`. AI enrichment accepts `AI_BASE_URL`, `AI_MODEL`, and either `AI_API_KEY_FILE` (preferred; file wins) or `AI_API_KEY`. Mimo defaults use `TTS_BASE_URL` (`https://api.xiaomimimo.com/v1`), `TTS_MODEL` (`mimo-v2.5-tts`), `TTS_VOICE` (`Chloe`), `TTS_AUDIO_DIR`, `TTS_TIMEOUT_SECONDS`, and `TTS_API_KEY_FILE`/`TTS_API_KEY`; 豆包 defaults use the original host-only `VOLC_TTS_BASE_URL` (`https://openspeech.bytedance.com`), then append the fixed agent-plan path, plus `VOLC_TTS_MODEL`, `VOLC_TTS_RESOURCE_ID`, `VOLC_TTS_VOICE`, `VOLC_TTS_TIMEOUT_SECONDS`, and `VOLC_TTS_API_KEY_FILE`/`VOLC_TTS_API_KEY`. The System page can override each provider's URL and API key at runtime. `TRUSTED_LOCAL_WEB` defaults to `false`. Wildcard CORS origins are rejected. Set `LOG_LEVEL=DEBUG` to log route templates, latency, dictionary misses, and actor type — tokens and request bodies are never logged.
