...
**Runtime TTS settings (0009):** the System page's 本地词库语音 block also persists, in the `SystemAudioSetting` singleton, the **import auto-generate toggle** (`auto_generate_on_import`, overrides `TTS_AUTO_GENERATE_ON_IMPORT`) and the **volc tuning knobs** (`volc_resource_id` / `volc_speech_rate` / `volc_loudness_rate` / `volc_silence_ms`, overriding the `VOLC_TTS_*` env vars). Every TTS consumer resolves config through `system_settings.audio_runtime_settings()` (env snapshot + non-null DB overrides), so changes apply at runtime without redeploying; NULL columns fall back to env. The import worker's auto-enqueue check reads the same runtime settings. `GET /system/audio-settings` returns the effective `auto_generate_on_import` + `volc_tuning` (non-secret); api keys are still only ever returned masked.
