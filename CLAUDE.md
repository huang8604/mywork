...
Words can carry server-generated MP3 pronunciation metadata (`audio_path`, `audio_format`, `audio_voice`, `audio_generated_at`, `audio_bytes`). The current System page exposes one custom OpenAI-compatible TTS connection: API URL + API key, with model/voice inherited from the server runtime; the key is masked in responses and never sent to Vue. `synthesize_word_mp3(text, *, provider=None)` resolves that custom connection and does not fall back to another account. Historical mimo/volc columns and provider arguments remain only for rolling migration and already queued jobs.

- **Custom endpoint**: `POST {TTS_BASE_URL}/chat/completions` (or the URL entered in System settings) with `modalities:["text","audio"]`; base64 MP3 is read from `choices[0].message.audio.data`. The server keeps the custom API key in the database/runtime layer and only returns a mask. `TTS_MODEL` and `TTS_VOICE` remain server-side defaults.


Admin audio routes (all `words:*`, never exposed to students — students lack `words:read`): word generation, batch generation, number announcements, and progress remain background operations. They resolve the single custom TTS connection saved in `/system/audio-settings`; old `provider` request fields and `/words/audio/providers` remain accepted only for rolling compatibility. The API key is never returned to Vue.
...
**Runtime TTS settings (0010):** the System page persists one custom API URL and API key in the `SystemAudioSetting` singleton, plus the import auto-generate toggle (`auto_generate_on_import`, overriding `TTS_AUTO_GENERATE_ON_IMPORT`). API keys are write-only from the UI and are only returned masked; model and voice remain server-side runtime defaults. Historical provider columns are retained for safe rolling migration but are not exposed as account settings.
...
`Settings.from_env()` (`core/config.py`, `lru_cache`d) is the single source for config. Required for startup: `API_TOKEN_PEPPER` or `API_TOKEN_PEPPER_FILE` (≥32 bytes). Production also needs `DATABASE_URL`, `PUBLIC_BASE_URL`, `TRUSTED_HOSTS`, `TRUSTED_PROXY_CIDRS`. AI enrichment accepts `AI_BASE_URL`, `AI_MODEL`, and either `AI_API_KEY_FILE` (preferred; file wins) or `AI_API_KEY`. Cloud TTS runtime defaults accept `TTS_BASE_URL` (default `https://api.xiaomimimo.com/v1`), `TTS_MODEL` (default `mimo-v2.5-tts`), `TTS_VOICE` (default `Chloe`), `TTS_AUDIO_DIR`, `TTS_TIMEOUT_SECONDS`, and either `TTS_API_KEY_FILE` (preferred; file wins) or `TTS_API_KEY`; the System page can override only the custom API URL and API key at runtime. `TRUSTED_LOCAL_WEB` defaults to `false`. Wildcard CORS origins are rejected. Set `LOG_LEVEL=DEBUG` to log route templates, latency, dictionary misses, and actor type — tokens and request bodies are never logged.
