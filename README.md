...
- 新生成的复习表默认为**未开始**；可手动切换为未开始、进行中、已完成或已归档。开始回录后自动进入进行中，整轮完成后自动进入已完成。
...
- 仅列出状态为**进行中**的复习表；支持英文英音和中文普通话两种默写语言。
- 默认每题结束后间隔 6 秒自动播放下一题，序号 1–50 始终播报；运行中可暂停、继续、重播、跳过或手动播放下一题。
- 中文释义与中文序号在首次播放时自动生成并缓存云端 MP3，失败时自动回退浏览器语音。
...
- **本地词库语音导入**：在后台为本地词库全部单词生成共享 MP3，显示“已生成 / 总词数”；支持暂停、恢复、失败续跑和定时扫描。模型额度耗尽时等待 5 小时后自动继续，之后新增且命中本地词库的单词直接复用已有音频。
...
- **student**：可查看自己的「概览」并使用「在线复习」，看不到词库、复习表管理、历史与系统管理；复习统计只显示本人结果。
