...
- 所有单词音频入口都会成对生成英文单词与中文释义 MP3；支持分别播放、一键补齐缺失音频、批量补齐及全部重新生成。序号 1–50 也会同时生成中英文。
...
- 错词按历史累计定义：单词只要曾经记录过“不认识”，即使之后答对，仍会继续进入错题候选用于巩固。
- **自定义选词**：直接挑选一组单词生成，保留选择顺序。
- **从 TXT 导入并生成**：粘贴或选择一份单词文本，导入后直接生成复习表。
- 每张复习表保存策略、随机种子、题目快照与入选原因，**可复现**。
- 新生成的复习表默认为**未开始**；可在复习表列表直接切换为未开始、正在进行、已完成或已归档。开始回录后自动进入正在进行，整轮完成后自动进入已完成。

### 复习表会话（纸面主路径）

- 「复习表 / 答案 / 结果回录」三个标签：复习表用于纸面默写，答案用于核对，结果回录用于录入三态。
- **打印**：有自定义标题时以该标题作为打印名称；支持中英两种留空回忆方向，单词与 `/音标/` 同行、例句全显示、留空侧不画线；标准字号按「约 20 词 / 一页 A4」校准。
- 编辑复习表时使用“搜索添加 + 当前清单”双区操作，可直接删除单词和上下调整打印顺序。
...
- 仅从状态为**正在进行**的复习表中选择，逐张卡片复习并记录三态，最后一题给出本轮汇总。
- 可查看当前账号今日的复习结果。

### 在线默写

- 仅列出状态为**正在进行**的复习表；支持英文和中文两种默写语言。
- 默认每题结束后间隔 6 秒自动播放下一题，序号 1–50 始终播报；运行中可暂停、继续、重播、跳过或手动播放下一题。
- 单词和序号的中英文 MP3 可提前成对生成；中文释义与中文序号缺失时仍会在首次播放时自动生成并缓存，失败时自动回退浏览器语音。
- 完成本轮后在汇总下方显示全部听写词及草稿，并统一标记为“已听写”。

### 历史与统计

- 完整复习流水，支持按单词 / 结果 / 来源 / 操作者 / 复习表 / 日期筛选与分页。
- 任意一条记录都可事后纠正，系统在同一事务内重建该词统计。间隔规则 `(1,3,7,14,30)` 天，按连续认识次数推进。
- 今日概览用 365 天贡献图展示每日背诵次数，支持月份/星期定位、每日明细和移动端横向浏览。
...
- **问题与需求记录**：管理员可随时保存待修复问题、复现步骤和后续需求；内容保存在数据库中并随整库备份。
